...
public class LotsOfBallsDestructable implements Ball, Animated{
	float x, y;
	int color;
	...
	public LotsOfBallsDestructable(World w, float x, float y) {
		this.world = w;
...
if(x<-2||x>world.getMaxX()+2||y<-2||y>world.getMaxY()+2){
		...
		counter++;
		if(counter==5){
			world.addBallDuring(new Bouncer(world, x, y));
			counter=0;
		}
		if(world.checkNumBalls()>100){
			world.removeBall(this);
		}
	}
}
